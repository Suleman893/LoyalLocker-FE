...
import { isAfter, differenceInSeconds, format } from "date-fns";
...
const OfferTimeCounter = ({ expiryDate }) => {
...
const OfferEarnRuleCard = ({ itm, onView }) => {
  ...
  return (
    <div style={cardStyle}>
      ...
      {itm?.status === "ACTIVE" && itm?.endAt && (
          <OfferTimeCounter expiryDate={itm?.endAt} />
        )}
        </div>
...
export default OfferEarnRuleCard;
